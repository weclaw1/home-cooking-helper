...
export const homeProductsStore = {
  ...
  addProduct(product: Product) {
    if (this.containsProduct(product.name)) {
      throw new Error(`Home products list already contains product with name ${product.name}`);
    }
  ...
  deleteProduct(productName: string) {
    const productIndex = this.state.homeProducts.findIndex(product => product.name === productName);
    if (productIndex < 0) {
      throw new Error(`Home products list does not contain product with name: ${productName}`);
    }
  ...
  containsProduct(productName: string): boolean {
    return this.state.homeProducts.some(product => product.name === productName);
  }
}
